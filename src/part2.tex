...
因为绝大部分 \LaTeX{} 发行版中的 \file{graphics.cfg} 可以自动识别
\footnote{\file{graphics.cfg} 文件会检测文件的编译方式。
...
在这两种方式之间切换会改变两件事情。
\footnote{使用 \prgname{xelatex} 或者 \prgname{lualatex} 编译时无需考虑此问题。
...
从而可以在 \prgname{pdflatex} 编译方式下使用。
...
\footnote{实际上在最新版本（2016年）中，
	\file{dvips.def} 中定义扩展名的方式为
	\begin{flushleft}
