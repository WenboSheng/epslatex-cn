...
注意，由于最新的\pkg{ctex}宏集不支持 \prgname{dvips}，
需要手动使用\pkg{CJK}。
\begin{lstlisting}
%\usepackage{CJKutf8}
\begin{CJK}{UTF8}{gkai}
...
它的具体用法已在前面几小节中介绍过了。
此外，\pkg{PSfrag} 宏包还提供 \cmd{tex} 直接将 \LaTeX{} 文本嵌入到 \file{eps} 中。
...
\subsubsection{PSfrag和 \pdfTeX{}}

\pkg{psfrag} 不能在 \pdfTeX{} 模式下使用。
如果需要进行 \pkg{psfrag} 替换，
一种解决办法是使用 \LaTeX{}-\file{dvi}-PostScript-\file{pdf} 编译途径。
这样虽然可以使用 \pkg{psfrag} 替换，然而同时也失去了 \pdfTeX{} 提供的优势。
另外一种方法是在 \pdfTeX{} 中间接地使用 \pkg{psfrag}，
这样既可以进行 \pkg{psfrag} 替换，又可以利用 \pdfTeX{} 的优势，
不过，这种方法比较费时费力。
\begin{enumerate}
...
\end{verbatim}
	\item 在操作系统的命令行中执行以下步骤\footnote{%
		这里使用 \TeX Live 自带的 \prgname{ps2eps} 程序代替原文中的第三方程序 \prgname{epstool}。
		——译注}：
\begin{lstlisting}[language=bash]
...
...
\end{verbatim}
	\item 使用 \prgname{pdflatex} 处理 \LaTeX{} 文件。
\end{enumerate}
