...
\begin{description}
	...
	不要让 \cmd{textfraction} 的值小于~0.15，
	因为这样的文本页难以阅读。
...
\begin{table}
	...
	\begin{tabular}{>{\columncolor{morelight}}l|>{\CJKfamily{kai}}m{10cm}|}
		...
		\cline{2-2}
		\tbltt{floatsep} & 出现在页面的顶部或底部的浮动体之间的垂直距离。
		缺省为~\texttt{12pt plus 2pt minus 2pt}。 \\
		\cline{2-2}
		\tbltt{textfloatsep} & 出现在页面的顶部或底部的浮动体与文本之间的垂直距离。
		缺省为~\texttt{20pt plus 2pt minus 4pt}。 \\
		\cline{2-2}
		\tbltt{intextsep} & 出现在页面中间的浮动体（如使用了~\texttt{h}~选项
		的浮动体）与上下方文本之间的垂直距离。
		缺省为~\texttt{12pt plus 2pt minus 2pt}。 \\
		\cline{2-2}
	\end{tabular}
...
表~\ref{tab:figlength}~中给出的长度不会影响浮动页上各浮动体之间
...
\begin{table}
	...
	\begin{tabular}{>{\columncolor{morelight}}l|>{\CJKfamily{kai}}m{10cm}|}
		...
		\cline{2-2}
		\tbltt{@fptop} & 浮动页中顶部的浮动体上方的空白。
		缺省为~\texttt{0pt plus 1.0fil}。 \\
		\cline{2-2}
		\tbltt{@fpsep} & 浮动页中的浮动体之间的距离。
		缺省为~\texttt{8pt plus 2.0fil}。 \\
		\cline{2-2}
		\tbltt{@fpbot} & 浮动页中底部的浮动体下方的空白。
		缺省为~\texttt{0pt plus 1.0fil}。 \\
		\cline{2-2}
	\end{tabular}
\end{table}
...
\begin{table}
	...
	\begin{tabular}{>{\columncolor{morelight}}l|>{\CJKfamily{kai}}m{10cm}|}
		...
		\cline{2-2}
		\tbltt{topfigrule} & 在一页顶部的最后一个浮动体后，~\cmd{textfloatsep}~前
		被执行的命令（见第~\ref{sec:vspace}~节）。 \\
		\cline{2-2}
		\tbltt{bottomfigrule} & 在一页底部第一个浮动体前，~\cmd{textfloatsep}~后
		被执行的命令。\\
		\cline{2-2}
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	一个浮动体。这可通过修改~\ci{efloatseparator}~来改变。例如，
	\begin{Verbatim}[xleftmargin=1cm]
...
\textsf{caption2}~宏包可以和很多与浮动体有关的宏包一起使用。
