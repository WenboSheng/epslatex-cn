...
\begin{description}
	...
	\footnote{当一幅图形被放置在页面的底部时，
		...
		现在还没有办法来避免这种情况。}。
	\item [p] \emph{\CJKfamily{hei} 浮动页。} 将图形放置在允许有浮动对象的页面上。
\end{description}
...
\begin{enumerate}
	...
	例如，一页上的浮动对象的数目不能超过 \opt{totalnumber}\index{totalnumber@\texttt}。
...
由于这一队列只能有 18 个位置，
所以当未处理的浮动对象的数目超过这一限制时就会导致发生“Too Many Unprocessed Floats”的错误。
造成这种错误有四种可能的原因：
...
\begin{enumerate}
	...
	\item 在很少的情况下，如使用了很多浮动对象和 \cmd{marginpar} 边注
	（会和浮动体使用相同的未处理队列机制相同），
	可能确实需要较长的等待队列。
	...
	\begin{enumerate}
		\item 将图形散布在正文中。
		这会使得有足够的文本来自然分页，\LaTeX{} 也会更容易地处理浮动对象。
