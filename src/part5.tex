...
\section{堆叠的图形和子图}\label{sec:stackedfigs}
...
\subsection{堆叠的图形}\label{ssec:stackedfigs}
...
并且使用 \cmd{ref}、\cmd{ref*} 以及 \pkg{subcaption} 的 \cmd{subref}、\cmd{subref*} 对整个图形和每一子图进行标签引用
\footnote{
	\cmd{ref*} 由 \pkg{hyperref} 宏包提供，\cmd{subref*} 同样需要 \pkg{hyperref} 宏包的支持。}，
相应的效果见表~\ref{tab:ref-subfig}。

本节简要介绍 \pkgi{subcaption} 宏包，
...
\end{lstlisting}
	...
	为方便起见，\pkg{subcaption} 宏包提供了 \envi{subfigure} 和 \envi{subtable} 两个环境用于创建子图和子表格，用法与 \env{minipage} 环境完全相同。
...
\end{lstlisting}
	其中各个选项的含义参见第~\ref{ssec:captioncmd} 节以及 \pkg{caption} 宏包文档\cite{caption-doc}。
	...
	例如在 \env{subfigure} 和 \env{subtable} 环境内的 \cmdOM{captionsetup}{sub}{\metacmd{options}}。
\end{itemize}
...
\end{wrapfigure}
\noindent \texttt{rapfig} 宏包提供了一个 \envi{wrapfigure} 环境\footnote{
	\pkg{wrapfig} 也同时提供了一个 \envi{wraptable} 环境。}来排版窄小的图形，
...
\begin{description}
	\item [\opt{r,R}] 表示图形位于文本的左边。
	\item [\opt{l,L}] 表示图形位于文本的右边。
...
\begin{itemize}
	...
	\item 不能在任何列表环境中使用 \env{wrapfigure}。
...
还有两个变体 \envi{figwindow} 和 \envi{tabwindow}。
...
\begin{figwindow}[1,r,{\mbox{%
		\includegraphics[width=4cm]{tiger}}},{Tiger}]
	可是哈卜拉姆再聪明...可是我偏不喜欢。」
\end{figwindow}
\end{Verbatim}

\begin{figwindow}[1,r,{\mbox{%
		\includegraphics[width=4cm,clip]{tiger}}},{Tiger}]
	可是哈卜拉姆再聪明、再有学问，有一件事却是他不能解答的，因为包
	罗万有的「可兰经」上也没有答案；如果你深深爱著的人，却深深的爱上了
	别人，有甚麽法子？白马带著她一步步的回到中原。白马已经老了，只能慢
	慢的走，但终是能回到中原的。江南有杨柳、桃花，有燕子、金鱼……汉人中
...
\begin{figwindow}[1,c,{\mbox{%
		\includegraphics[width=3cm]{tiger}}},{Tiger}]
	可是哈卜拉姆再聪明...可是我偏不喜欢。」
\end{figwindow}
\end{Verbatim}

\begin{figwindow}[1,c,{\mbox{%
		\includegraphics[width=3cm,clip]{tiger}}},{Tiger}]
	可是哈卜拉姆再聪明、再有学问，有一件事却是他不能解答的，因为包
	罗万有的「可兰经」上也没有答案；如果你深深爱著的人，却深深的爱上了
	别人，有甚麽法子？白马带著她一步步的回到中原。白马已经老了，只能慢
	慢的走，但终是能回到中原的。江南有杨柳、桃花，有燕子、金鱼……汉人中
